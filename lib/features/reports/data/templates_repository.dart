...
class TemplatesRepository {
  ...
  static const _starterSeedKey =
      'templates.starter_seed.v1';

  Future<SharedPreferences> get _prefs async =>
      SharedPreferences.getInstance();

  String _key(String templateId) =>
      '$_prefix$templateId';
  ...
  Future<TemplateDoc> loadTemplate(
    String templateId,
  ) async {
    await _ensureStarterTemplatesSeeded();

    final local =
        await _loadLocalTemplateOrNull(
      templateId,
    );
    ...
    final remote =
        await _loadRemoteTemplateOrNull(
      templateId,
    );

    if (remote != null) {
      await _cacheTemplateLocally(
        remote,
      );
  ...
  Future<void>
  updateTemplateRecordFieldSettings({
    required String templateId,
    required Set<String>
    saveToRecordsSectionIds,
  }) async {
    final template =
        await loadTemplate(
      templateId,
  ...
  Future<void> deleteTemplate(
    ...
    await _deleteRemoteTemplate(
      templateId,
    );
  }

  Future<List<TemplateSummary>>
  listTemplates() async {
    await _ensureStarterTemplatesSeeded();

    final local =
        await _listLocalTemplates();

    final remote =
        await _listRemoteTemplates();
  ...
  Future<void> _cacheTemplateLocally(
    TemplateDoc template,
  ) async {
    final prefs = await _prefs;
  ...
  Future<void> _deleteRemoteTemplate(
    String templateId,
  ) async {
    if (Firebase.apps.isEmpty) {
      return;
  ...
  Future<void>
  migrateCloudTemplatesToSignedInUser()
  async {
    if (Firebase.apps.isEmpty) {
      return;
    }
    ...
    try {
      ...
      if (!identity.isSignedInUser ||
          identity.authUid == null) {
        return;
      }
